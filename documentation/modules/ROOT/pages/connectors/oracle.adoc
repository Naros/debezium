...
The connector processes the signal on its streaming thread at the start of the next mining iteration, so that the buffer is modified in a consistent state.
On a busy connector the transaction is therefore removed a few seconds after the signal is received, and the connector log reports whether it was found and dropped.
...
[id="oracle-log-pending-transactions-signal"]
=== Logging pending transactions

You can use {prodname} signaling to send a `log-pending-transactions` signal to the connector at run time to write the details of every transaction that is currently pending in the LogMiner buffer to the connector log.

The connector's mining window is bounded by the oldest transaction that it has seen but that has not yet committed or rolled back.
Identifying these transactions normally requires that you enable verbose logging before the situation occurs, or that you have access to query LogMiner directly.
The `log-pending-transactions` signal provides an on-demand view of the transactions that the connector currently tracks, so that you can decide whether to wait for a transaction, or remove it by using the xref:oracle-drop-transaction-signal[`drop-transaction`] signal.

The signal reports two kinds of transactions:

Active transactions:: Transactions for which the connector has buffered one or more change events.
Deferred transactions:: Transactions that the connector has seen start, but for which it has not yet buffered any change events.

For each transaction, the connector logs the transaction ID, the SCN and time at which the transaction started, the database user name and client identifier associated with the transaction, and the redo thread on which the transaction was mined.
For active transactions, the connector also logs the number of buffered events.
Entries are ordered from the oldest transaction to the newest, so that the first entry is the transaction that currently holds back the mining window.

The connector processes the signal on its streaming thread at the start of the next mining iteration, so that the buffer is read in a consistent state.
On a busy connector the entries can therefore appear a few seconds after the signal is received.

.Prerequisites
* The connector is configured to use one of the available link:{link-signalling}#sending-signals-to-a-debezium-connector[{prodname} signaling channels].
* You have access to the connector log file.

.Procedure
* Send a signal that includes the standard `id` and `type` properties of a {prodname} signal to your preferred signaling channel.
The signal does not require any `data`.
+
.Example SQL using the `source` signal channel
[source,sql,indent=0,subs="+attributes"]
----
INSERT INTO debezium.debezium_signal (id, type) VALUES('log-tx-1', 'log-pending-transactions');
----
+
.Example message payload using the `kafka` signal channel
[source,json,indent=0,subs="+attributes"]
----
{
  "type": "log-pending-transactions"
}
----
+
The connector writes entries similar to the following to its log:
+
----
Pending transactions in Oracle LogMiner buffer as requested by signal 'log-tx-1': 2 total (1 active, 1 deferred)
Active transaction 1a.2b.001f: startScn=12345678, changeTime=2024-01-01T10:15:30Z, userName=APP, clientId=batch-loader, redoThread=1, events=1500
Deferred transaction 1c.2d.0020: startScn=12345900, changeTime=2024-01-01T10:16:02Z, userName=APP, clientId=null, redoThread=1
----
...
Most of these programs manipulate the LOB column in totality, however, some programs, such as `WRITE`, `WRITEAPPEND`, `TRIM`, and `ERASE`, manipulate only a subset of the LOB data buffer.

When using XStream, each invocation of one of these programs emits a dedicated `LOB_WRITE`, `LOB_TRIM`, or `LOB_ERASE` logical change record (LCR).
Such an LCR carries only the portion of the LOB that the program changed, not the complete column value.

The connector emits each of these LCRs as an update change event.
Because the LCR does not contain the full column value, the connector re-selects the current value of the row's LOB columns from the database, using the primary key of the row and the commit SCN of the event, and places the complete values in the `after` element of the change event.
The `before` element contains the unavailable value placeholder for the LOB columns, as described in the xref:oracle-property-unavailable-value-placeholder[`unavailable.value.placeholder`] property.

For the re-selection to succeed, the table must have a primary key, and the connector must be able to read the table.
If the table has no primary key, or the row was deleted before the connector could re-select it, the connector logs a warning and emits the unavailable value placeholder for the LOB columns in the `after` element.
